...
var toggleBarMouseDown = function(e, accordionConstructor) {
    ...
    function startDragging() {
        ...
        accordionBox = lib.findHost(toggleBlock, accordionConstructor);
        ...
        lib.setBox(toggleBlockDragging, toggleBlockRect.left, toggleBlockRect.top, toggleBlockRect.width, toggleBlockRect.height);
    ...
    var onMouseMove = function(e) {
        ...
        if (!accordionBox) {
            accordionBox = lib.findHost(e.target, accordionConstructor);
            if (accordionBox) {
                accordionBoxRect = accordionBox.element.getBoundingClientRect();

                nextBlock = lib.findNode(e.target, "toggle-block");
                if (nextBlock) {
...
var toggleBarOnClick = function(e) {
    var toggleBlock = lib.findNode(e.target, "toggle-block");
    if (!toggleBlock)
        return;
...
var splitterMouseDown = function(e) {
    var button = e.button;
    if (button !== 0)
        return;

    var splitter = lib.findNode(e.target, "splitter");
    if (!splitter)
        return;
...
exports.toggleBarMouseDown = toggleBarMouseDown;
exports.toggleBarOnClick = toggleBarOnClick;
exports.splitterMouseDown = splitterMouseDown;
